...
def test_func(model, dataloder, save_path, exp_name, dataset, data_name): #save_path: log/ist_net_default/REAL275/eval_epoch30
    model.eval()
    with tqdm(total=len(dataloder)) as t:
        for i, data in enumerate(dataloder):
            ...
            #                                                                     iou_thres_list)
            draw_box_to_image(data, result, exp_name, dataset, data_name, i)
...
def draw_box_to_image(data, result, exp_name, dataset, data_name, img_id):
    synset_names = ['BG',  # 0
                    'bottle',  # 1
                    'bowl',  # 2
                    'camera',  # 3
    ...
    out_dir = os.path.join('/data/jhq/IST-Net/log', exp_name , dataset, 'vis_img')
